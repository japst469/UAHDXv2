...
{
    public static string Send(string[] mail_creds, string toList, string from,string[] username, byte[] score)
    {
        int i;                      //Generic counter variables
        int Entries=score.Count()-1; //# of players
        ...
        for (i = 0; i <= Entries; i++)
        {
            body += username[i] + ": " + score[i].ToString();   //"Name: Score"
            body += "<br />";                                   //Carriage return
        }
        body += "<br />";

        //Get the HiScore and the name(s)
        HiScore=score.Max();    //Get HiScore
        //Find index(es) of Winner(s), then get the name(s)
        for (i = 0; i <= Entries; i++)
        {
            if (score[i]==HiScore)  //If equal to the HiScore
            {
                Ties++; //Increment Ties
